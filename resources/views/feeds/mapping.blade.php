...
@section('content')
    ...
    @include('components.titlebar')
        ...
        <div class="container">
            <div class="row">
                <form action="/feeds/{{$feed->id}}/mapping" method="post">
                                    ...
                                    @foreach($requiredFields as $field)
                                        <tr>
                                            ...
                                            <td>
                                                <select name="map[{{$field->field}}]" class="field_{{$field->field}}">
                                                    ...
                                                    <option value="custom"
                                                            ...
                                                            @if(is_array($channelmappings) && isset($channelmappings[$field->field]) && $channelmappings[$field->field]['source'] == "custom") selected @endif>
                                                                @lang('marketplaces.Custom value')
                                                    </option>
                                                    @foreach($productColumns as $column)
                                                        <option value="{{$column}}"
                                                                @if(!is_array($channelmappings) && $field->field == $column) selected @endif
                                                                @if(is_array($channelmappings) && isset($channelmappings[$field->field]) && $channelmappings[$field->field]['source'] == $column) selected @endif
                                                        >{{$column}}</option>
                                                    @endforeach
                                            ...
                                            <td><input type="text" name="value[{{$field->field}}]"
                                                       @if(is_array($channelmappings) && isset($channelmappings[$field->field]) && $channelmappings[$field->field]['source'] == 'custom')
                ...
                <div class="button-set padding">
                    <button type="submit" class="btn btn-primary">@lang('connections.Save mapping &amp; process products')</button>
                </div>
